...
| 词汇与短语 | 每日 20 个新词、到期复习独立计数、首次学习、即时检测、答错延迟复现、构词/词族记忆、退出续学 |
| 作文 | 作文模板与个人作文管理 |
| 学习记录 | 今日学习时长与阶段统计 |
| 账户 | 注册、登录、JWT 鉴权 |
...
移动端统一位于 `/m/*`，主导航为任务、检索、我的；词汇、短语、作文、真题和科目页均属于“我的”功能域。移动端只负责学习与查看，学习时间和每日单词目标统一在桌面后台配置。
...
当前待部署版本必须按顺序执行 [每日词单与配置集中管理](./releases/2026-08-11-stable-vocabulary-queue.md) 和 [零基础快速词汇模式](./releases/2026-08-11-rapid-vocabulary-mode.md) 的 Migration；完整验收与回滚步骤见两份发布说明。
...
1. 产品与技术说明只维护本文件和根目录 `README.md`；单次生产发布的操作清单统一放在 `docs/releases/`，并从 `docs/DEPLOYMENT.md` 建立入口。
